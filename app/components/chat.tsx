...
export function Chat({
  ...
  const fetchWithAbort = useMemo(() => {
    return async (input: RequestInfo | URL, init?: RequestInit) => {
      // Ensure we aren't reusing an already-aborted signal
      ...
      try {
        console.log('[Chat fetch] request', input, requestInit);
        const response = await fetchWithErrorHandlers(input, requestInit);
        console.log('[Chat fetch] response ok', {
  ...
  const {
    ...
    onFinish: ({ isAbort, isDisconnect, isError }) => {
      ...
      if (shouldResume && resumeAttemptCountRef.current < maxResumeAttempts) {
        console.log('[Chat onFinish] Resuming stream', {
          attempt: resumeAttemptCountRef.current + 1,
          lastPartType: lastPartRef.current?.type,
        });
