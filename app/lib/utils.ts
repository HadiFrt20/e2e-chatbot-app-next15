...
export async function fetchWithErrorHandlers(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      let parsedResponse: { code?: string; cause?: string } = {};
      try {
        parsedResponse = JSON.parse(bodyText);
      } catch (_) {
        // non-JSON response
      }
      console.error('[fetchWithErrorHandlers] non-OK response', {
      ...
      throw new ChatSDKError((code as ErrorCode) ?? 'offline:chat', cause);
    }
